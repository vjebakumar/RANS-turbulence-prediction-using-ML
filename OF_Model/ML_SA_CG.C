...
void ML_SA_CG<BasicTurbulenceModel>::run_ml_graph(double* mean_array, double* std_array, int num_inputs, int num_outputs)
{
    ...
    std::cout << "/* total number of cells */" << num_cells <<std::endl;
    ...
    volScalarField uxx_ = UMean_.component(vector::X);
    volScalarField uyy_ = UMean_.component(vector::Y);
    ...
    forAll(u_.internalField(), id) // for boundary field use u_.boundaryField()
    {
        ...
        float i5  = (uxx_[id] - mean_array[4])/(std_array[4]);
        float i6  = (uyy_[id] - mean_array[5])/(std_array[5]);
